...
def cached_all_users_df() -> pd.DataFrame:
    """users 시트 전체를 DataFrame으로 로드 (교구/직분/이름 매핑용)"""
    s = get_storage()
    if not s:
        return pd.DataFrame(columns=USERS_REQUIRED)
    try:
        # 스키마 보장 (필요 컬럼 없으면 생성)
        s._ensure_schema()
    except Exception:
    ...
    try:
        ...
        # get_all_records 실패 시 fallback
        try:
            values = s.ws_users.get_all_values()
